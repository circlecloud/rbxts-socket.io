...
export abstract class Polling extends Transport {
     ...
     * @package
     */
    override pause(onPause: Callback) {
    ...
    override onData(data: string) {
        // $debug("onData", data);
        const callback = (packet: Packet) => {
            // $debug('onData callback', packet)
            // if its the first message we consider the transport open
            if ("opening" === this.readyState && packet.packetType === "open") {
                this.onOpen();
            }

            // if its a close packet, we close the ongoing requests
            if ("close" === packet.packetType) {
        ...
        for (const packet of decodePayload(data)) {
            callback(packet)
        }
    ...
    abstract doWrite(data: string, callback: () => void): void;
}