...
const SEPARATOR = string.char(30)// see https://en.wikipedia.org/wiki/Delimiter#ASCII_delimited_text
...
const encodePayload = (
    ...
    // const length = packets.size();
    // const encodedPackets = new Array<defined>(length);
    // let count = 0;
    const payload = packets.map(packet => encodePacket(packet, false)).join(SEPARATOR)
    // $debug("engine.io-parser encodePayload", packets, payload)
    return payload
...
const decodePayload = (
    ...
    for (let i = 0; i < encodedPackets.size(); i++) {
        const decodedPacket = decodePacket(encodedPackets[i]);
        packets.push(decodedPacket);
        if (decodedPacket.packetType === "error") {
            break;
        }
    }
    return packets;
