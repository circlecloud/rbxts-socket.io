...
import { BinaryType, Packet, PacketType, protocol, RawData } from "../engine.io-parser";
import { Error, JSON } from "../polyfill";
import { decode } from "./contrib/parseqs";
import { parse } from "./contrib/parseuri";
import { defaultBinaryType } from "./globals.node";
...
export class SocketWithoutUpgrade extends Emitter<any, any> {
    ...
    protected readonly transports: string[] = ["polling"];
    protected upgrading: boolean = false;
    protected setTimeoutFn: Callback = () => { };

    private _prevBufferLen: number = 0;
    private _pingInterval: number = -1;
    private _pingTimeout: number = -1;
    private _maxPayload: number = -1;
    private _pingTimeoutTimer: thread | undefined

    private _pingTimeoutTime = 100000000000;
    private clearTimeoutFn: Callback = () => { };
    ...
    constructor(uri: string, opts: Partial<SocketOptions> = {}) {
        super();
        if (uri) {
            const parsed = parse(uri)
            opts.hostname = parsed.host
            opts.secure = parsed.protocol === "https" || parsed.protocol === "wss"
            opts.port = parsed.port
            if (parsed.query) opts.query = parsed.queryKey
        } else if (opts.host) {
        ...
        for (const [name, clazz] of DEFAULT_TRANSPORTS as unknown as Map<string, TransportCtor>) {
            this.transports.push(name);
            this._transportsByName[name] = clazz;
        }
        this.readyState = undefined as unknown as 'opening' | 'open' | 'closed';
        ...
        this.opts = {
            ...
            ...opts
        } as BaseSocketOptions

        this.opts.path =
            this.opts.path +
            (this.opts.addTrailingSlash ? "/" : "");

        if (typeIs(this.opts.query, "string")) {
    ...
    protected createTransport(name: string) {
        ...
        const query: any = this.opts.query;
    ...
    protected setTransport(transport: Transport) {
        // $debug("setting transport %s", transport.getName());

        if (this.transport) {
            // $debug("clearing existing transport %s", this.transport.getName());
        ...
        transport
            ...
            .on("packet", (packet) => this._onPacket(packet))
  ...
  */
    ...
    private _onPacket(packet: Packet) {
        if (
            ...
            // $debug('socket receive: type "%s", data "%s"', packet.packetType, packet.data);
    ...
    protected onHandshake(data: HandshakeData) {
        // $debug("engine.io-client:transport", "handshake %j", data);
    ...
    private _resetPingTimeout() {
        ...
        // ROBLOXPATCH
        this._pingTimeoutTime = os.time() + delay;
     ...
     */
    private _onDrain() {
        // $debug("_onDrain", this.writeBuffer.size());
        // ROBLOXPATCH ignore check because will cache at sendQueue
    ...
    protected flush() {
        if (
            ...
            // $debug("flushing %d packets in socket", packets.size());
            this.transport.send(packets);
            // ROBLOXPATCH clear exist packets
            packets.clear()
            // $debug("flushing packets.clear()", packets.size());
            // keep track of current length of writeBuffer
            // splice writeBuffer and callbackBuffer on `drain`
            this._prevBufferLen = packets.size();
    ...
    private _getWritablePackets() {
        ...
        //     this.writeBuffer.size() > 1;
        if (!shouldCheckPayloadSize) {
            return this.writeBuffer;
        }
        let payloadSize = 1; // first packet type
        for (let i = 0; i < this.writeBuffer.size(); i++) {
            const data = this.writeBuffer[i].data;
            if (data) {
                payloadSize += utf8.len(data)[0] as number;
            }
            // ROBLOXPATCH Max payload process
    ...
    _hasPingExpired() {
        ...
        const hasExpired = os.time() > this._pingTimeoutTime;
        if (hasExpired) {
            ...
            //     this._onClose("ping timeout");
            // }, this.setTimeoutFn);
            task.spawn(() => this._onClose("ping timeout"))
        }
    ...
    private _sendPacket(
        ...
        // $debug("_sendPacket", packetType, data, options);
        if (typeIs(data, "function")) {
            fn = data;
            data = undefined;
        }
...
export class SocketWithUpgrade extends SocketWithoutUpgrade {
    ...
    override onOpen() {
        ...
        if ("open" === this.readyState && this.opts.upgrade) {
            ...
            for (let i = 0; i < this._upgrades.size(); i++) {
    ...
    private _probe(name: string) {
        ...
        let transport = this.createTransport(name) as Transport;
        let failed = false;
        ...
        const onTransportOpen = () => {
            ...
            transport.send([{ packetType: "ping", data: "probe" }]);
            transport.once("packet", (msg: Packet) => {
                if (failed) return;
                if ("pong" === msg.packetType && "probe" === msg.data) {
                    ...
                    // $debug('pausing current transport "%s"', this.transport.getName());
                    this.transport.pause(() => {
                        ...
                        this.setTransport(transport);
                        transport.send([{ packetType: "upgrade" }]);
        ...
        const onerror = (err: Error | string) => {
            const errr = new Error("probe error: " + err);
            errr.transport = transport.getName();

            freezeTransport();

            // $debug('probe transport "%s" failed because of error: %s', name, err);

            this.emitReserved("upgradeError", errr);
        };
        ...
        function onupgrade(to: Transport) {
            if (transport && to.getName() !== transport.getName()) {
                // $debug('"%s" works - aborting "%s"', to.getName(), transport.getName());
                freezeTransport();
            }
        }
...
export class Socket extends SocketWithUpgrade {
    ...
    constructor(uri?: string | SocketOptions, opts: SocketOptions = {} as SocketOptions) {
        const o = typeOf(uri) === "string" ? opts : uri as SocketOptions;

        if (
            !o.transports ||
            (o.transports && typeOf(o.transports[0]) === "string")
        ) {
            o.transports = (o.transports || ["polling"])
                .map((transportName) => DEFAULT_TRANSPORTS[transportName as string])
                .filter((t) => !!t) as unknown as string[];
        }
